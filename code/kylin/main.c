...
static void onChar(GLFMDisplay *display, const char *utf8) {
    printf("onChar\n");
}

static void onKeyboardVisibilityChanged(GLFMDisplay *display, bool visible, double x, double y, double width, double height) {
    printf("onKeyboardVisibilityChanged\n");
}

static void onSurfaceError(GLFMDisplay *display, const char *message) {
    printf("onSurfaceError\n");
    printf("message = %s\n", message);
}

static void onSurfaceCreated(GLFMDisplay *display, int width, int height) {
    printf("onSurfaceCreated\n");
    int w, h;
    double s;
    glfmGetDisplaySize(display, &w, &h);
    s = glfmGetDisplayScale(display);
    printf("w = %d, h = %d, s = %f\n", w, h, s);
...
static void onMemoryWarning(GLFMDisplay *display) {
    printf("onMemoryWarning\n");
}
